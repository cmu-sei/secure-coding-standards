...
This compliant solution uses the POSIX `strtok_r()` function, which is reentrant.  It relies on no static variables, always tokenizing the string in its 3rd `saveptr` argument.
...
int main(void) {
  ...
  thrd_t thr;
  if (thrd_success != thrd_create(&thr, bar, NULL)) {
    // Handle Error
  }
