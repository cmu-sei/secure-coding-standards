...
This noncompliant code example the POSIX `strtok_r()` function, which is reentrant.  It relies on no static variables, always tokenizing the string in its 3rd `saveptr` argument.  However, by permitting a [data race](/sei-cert-c-coding-standard/back-matter/bb-definitions#BB.Definitions-datarace)  on `str` via `saveptr`, this code violates [CON43-C. Do not allow data races in multithreaded code](/sei-cert-c-coding-standard/rules/concurrency-con/con43-c).
